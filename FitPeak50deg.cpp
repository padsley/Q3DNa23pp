...
double p0 = 3.06890e-01, p1 = 6.74406e-06, p2 = -4.98691e-10;//pos->Brho calibration
...
//     TH1F *histo = (TH1F*)gROOT->FindObjectAny("hPos__49");
    TFile *fNaF = TFile::Open("/home/padsley/data/Q3DNa23pp/rootfiles/run025_an.root");
    TTree *tNaF = (TTree*)fNaF->Get("readout");
    tNaF->Draw("pos>>histoNaF(620,0,2500)","pos>0","");
    TH1F *histoNaF = (TH1F*)gROOT->FindObjectAny("histoNaF");
    histoNaF->Draw("E");
    
    TF1 *fitty = new TF1("fitty",TailedGaussian,1900,2000,6);
    ...
    c1->SaveAs("figures/FitPeak0_50.png");
    
    fitty->SetRange(240,340);
    fitty->SetParameters(285,20.,-2.1,960,200,0);
//     fitty->SetParLimits(5,0.,0.);
    std::cout << "Ex = 23Na: 9072 keV" << std::endl;
    histoNaF->Fit(fitty,"BRME");
    
    c1->SaveAs("figures/FitPeak1_50.png");
    
    fitty->SetRange(800,870);
    fitty->SetParameters(840,25,-3,800,600,0);
//     fitty->SetParLimits(5,0.,0.);
    std::cout << "Ex = 8975.3(7)-keV state in 23Na" << std::endl;
    histoNaF->Fit(fitty,"BRME");
    
    c1->SaveAs("figures/FitPeak2_50.png");
// //     
//     fitty->SetRange(1110,1170);
//     fitty->SetParameters(1136,19,-9,525,110);
...
//     ge->SetPoint(ge->GetN(),9.94643e+01,0.310526);//9113(3)-keV state in 23Na
//     ge->SetPointError(ge->GetN()-1,3.44662e-01,0.000103446);
    
    ge->SetPoint(ge->GetN(),8.43415e+02,0.312234);//8975.3(7)-keV state in 23Na
    ge->SetPointError(ge->GetN()-1,3.02541e-01,2.37718e-05);
    
    ge->SetPoint(ge->GetN(),5.33215e+02,0.310333);//8871.9(5)-keV state in 16O
    ge->SetPointError(ge->GetN()-1,7.55782e-01,1.71274e-05);
    
    ge->SetPoint(ge->GetN(),2.85309e+02,0.308946);//9072(3)-keV state in 23Na
    ge->SetPointError(ge->GetN()-1,5.33389e-01,0.000102998);
    
    ge->SetPoint(ge->GetN(),1.95079e+03,0.318147);//8798.7-keV state in 23Na
    ge->SetPointError(ge->GetN()-1,5.26668e-01,2.66444e-05);
...
//     grint->SetLineColor(kGreen);
    ...
    c2->SaveAs("BrhoCalibration50deg.eps");
    c2->SaveAs("BrhoCalibration50deg.pdf");
    c2->SaveAs("BrhoCalibration50deg.png");
    c2->SaveAs("BrhoCalibration50deg.C");
...
//         (double Brho, double mass, double theta)
        ...
        gExErrors1sig->SetPoint(i,BrhoToExNaF(y,938.782980,50.),0);
        gExErrors1sig->SetPointError(i,0, 1000.*(BrhoToExNaF(y,938.782980,50.) - BrhoToExNaF(y + sig_y,938.782980,50.))); 
        ...
        gExErrors2sig->SetPoint(i,BrhoToExNaF(y,938.782980,50.),0);
        gExErrors2sig->SetPointError(i,0, 1000.*(BrhoToExNaF(y,938.782980,50.) - BrhoToExNaF(y + sig_y,938.782980,50.))); 
        
        x = grint3->GetPointX(i);
        y = grint3->GetPointY(i);
        sig_y = grint3->GetErrorYhigh(i);
        
        gExErrors3sig->SetPoint(i,BrhoToExNaF(y,938.782980,50.),0);
        gExErrors3sig->SetPointError(i,0, 1000.*(BrhoToExNaF(y,938.782980,50.) - BrhoToExNaF(y + sig_y,938.782980,50.))); 
        
        gEx3sig_ActualValues->SetPoint(i,BrhoToExNaF(y,938.782980,50.),1000.*(BrhoToExNaF(y,938.782980,50.) - BrhoToExNaF(y + sig_y,938.782980,50.)));
//         std::cout << "i = " << i << " y = " << y << "\t sig_y = " << sig_y << std::endl;
        ...
        c3->SaveAs("ExUncertaintyBand50deg.eps");
        c3->SaveAs("ExUncertaintyBand50deg.pdf");
        c3->SaveAs("ExUncertaintyBand50deg.png");
        c3->SaveAs("ExUncertaintyBand50deg.C");
        Draw23NaExSpectrum();
}
...
//     TH1F *histoNaF = (TH1F*)gROOT->FindObjectAny("histoNaF");
    ...
    tNaF->Draw("BrhoToExNaF(Brho,938.782980,50.)>>hEx23Na(450,8.7,9.15)","pos>0 && pos<2500","");
    ...
    hEx23Na->GetYaxis()->SetRangeUser(1,4500);
    c4->Update();
    
    TFile *fC = TFile::Open("/home/padsley/data/Q3DNa23pp/rootfiles/run027_an.root");
    TTree *tC = (TTree*)fC->Get("readout");
    tC->SetAlias("Brho",Form("%e + %e * pos + %e * pos*pos",p0,p1,p2));
    tC->Draw("BrhoToExNaF(Brho,938.782980,50.)>>hEx19F(450,8.7,9.15)","pos>0 && pos<2500","goff");
    ...
    TGaxis *EcmAxis = new TGaxis(Sp,4500,Sp+1.e-3*axisLength,4500,0,axisLength,510,"L-");
...
//     b11->Draw("same");
    ...
    */
    c4->Update();
    c4->SaveAs("Na23_Ex_50deg_PA.eps");
    c4->SaveAs("Na23_Ex_50deg_PA.pdf");
    c4->SaveAs("Na23_Ex_50deg_PA.C");
}
